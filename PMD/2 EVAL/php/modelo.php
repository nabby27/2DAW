...
class Client implements \JsonSerializable {
    ...
    function save($link): ?Client {
        try {
            ...
            $result->bindParam(':admin', $this->admin);
            $result->execute();
        } catch (PDOException $e) {
            return null;
        }
...
class Order implements \JsonSerializable {
    ...
    function saveOrder($link): ?Order {
        try {
            ...
            $result->bindParam(':dniClient', $this->dniClient);
            $result->execute();
        } catch (PDOException $e) {
            return null;
        }
...
class LineOfOrder implements \JsonSerializable {
    ...
    function updateLine($link): ?LineOfOrder {
        try {
            ...
            $result = $link->prepare($queryString);
            $result->bindParam(':orderId', $this->orderId);
            $result->bindParam(':lineId', $this->lineId);
...
class ShoppingCart implements \JsonSerializable {
    ...
    function getNumberOfItemsByClient($link): ?int {
        try {
            ...
            $result = $link->prepare($queryString);
            $result->bindParam(':dniClient', $this->dniClient);
            $result->bindParam(':tempClientId', $this->tempClientId);
            $result->execute();
        } catch (PDOException $e) {
            return null;
        }
    ...
    function addToCart($link): ?bool {
        try {
            $queryString = "INSERT INTO shopping_cart (date, dni_client, temp_client_id, product_id, quantity) VALUES (:date, :dniClient, :tempClientId, :productId, :quantity)";
            $result = $link->prepare($queryString);
            $result->bindParam(':date', $this->date);
            $result->bindParam(':tempClientId', $this->tempClientId);
            $result->bindParam(':dniClient', $this->dniClient);
    ...
    function getProductsOnShoppingCart($link): ?array {
        ...
        while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
            try {    
                ...
                $resultProduct->bindParam(':productId', $row['product_id']);
